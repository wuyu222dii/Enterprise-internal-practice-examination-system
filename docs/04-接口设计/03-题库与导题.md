...
顶层 `confirmPendingHierarchy` 与 `hierarchyConfirm.confirmPendingHierarchy` 等价，服务端两种都认。

确认前服务端**重新校验**最新题库状态（FR-IMP-04）；可导入行集合相对预览变化则整单失败，任务进入 `needs_revalidation`，返回 `IMP_PREVIEW_STALE`。过期任务无确认入口。

事务全成全败；重复确认返回原入库结果。

### 5.4 任务状态

实现使用：`preview_ready` → `completed` | `cancelled` | `expired`；预览依据变化时转为 `needs_revalidation`，需 `POST /import/tasks/{id}/revalidate`（会重读 `fileKey` 并重算预览）。

`cancelled` 仅允许非终态（`preview_ready` / `needs_revalidation`）。过期任务只提供重新校验，不提供确认。

列表 `GET /import/tasks` 支持 `questionBankId`、`status` 筛选。
