...
| POST | `/admin/exams/{id}/pause` | FR-OPS-01 | AD-13 |
...
| GET | `/exams/{id}/active-attempt` | FR-ATT-02 | EX-03 |
...
| rules | `/wizard/rules` | 时长、次数、及格分；`paperMode=rules` 时为原子抽题规则行，`paperMode=fixedBank` 时指定已组卷题库整库入卷 |
...
本人被分配的正式考试列表；含五域状态摘要、`examCode`（考试码）、`portalUrl`（电脑端地址）与规则摘要（时长/次数/停止开卷时间）。

### 3.2 GET /exams/locate?examCode=

按考试码定位本人可参加的考试并返回与任务详情相同的脱敏对象。有效但无资格、以及无效码，统一 `404`「未找到可参加的考试」，不泄露考试是否存在。

### 3.3 GET /exams/{id}

任务详情：规则摘要、时段、剩余次数、电脑端地址、考试码、受控结果入口。必须校验当前员工应考分配；未分配时与定位失败使用同一提示，不泄露详情。
...
客户端倒计时必须基于 `expiresAt - serverNow` 定期校准。到期后 `remainingSeconds=0`，进入 20 秒观察窗，`observationRemainingSeconds` 递减；此期间保存/手动交卷返回 `409 ANS_IN_OBSERVATION`，尝试状态仍为 `inProgress`，不得显示已提交。观察窗结束且考试未暂停后，调度器取得唯一 timeout 交卷权。
...
- 尝试到期后 20s **故障观察窗**：期间保存/手动交卷返回 `409 ANS_IN_OBSERVATION`，状态不得变为已提交；无故障且观察结束才 timeout 交卷。暂停时观察窗不交卷。
- 到达 `stopAttemptAt` 后有效 `lifecycle=closing`，再等 20s 整场结束观察；暂停、待处置故障或在途尝试时不得进入 `ended`。
- 观察窗失守（本应补时的尝试已 timeout 交卷）时 `resultLocked=true`，结果接口 `resultState=locked`，不返回官方分数或逐题。
- 员工开卷失败码：`ATT_NOT_STARTED` / `ATT_NOT_ASSIGNED` / `ATT_NO_REMAINING_OPPORTUNITY` / `ATT_EXAM_PAUSED` / `ATT_WINDOW_CLOSED`。
...
### POST /admin/exams/{id}/pause

由授权管理员将考试转入暂停（配合故障处置）。生产路径优先 `POST /admin/outage-events/detect`。
