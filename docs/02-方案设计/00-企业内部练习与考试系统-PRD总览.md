...
**不纳入：** 字段级数据库、技术栈、服务拆分、部署拓扑、视觉稿、可点击原型、增长埋点，以及需求基线第 3.4 节明确排除的功能。

**接口规格：** 见 [接口设计文档 · API 设计说明](../04-接口设计/00-API设计说明.md) 与 [openapi.yaml](../04-接口设计/openapi.yaml)。

**视觉稿与研发标注：** 见 [研发对接文档 · Figma 设计源说明](../03-研发对接/00-Figma设计源说明.md)。
...
| FR-QST-04 | 题型与纯文本内容 | 单选、多选、判断、短文本解答题；纯文本换行；客观题全匹配，解答题去空白后精确比对（无人工阅卷） |
...
后续产品信息架构和原型沿用页面 ID；研发拆解沿用 FR 与 SYS ID；测试用例沿用需求基线验收 ID和页面验收 ID。数据库、技术方案需要实现本 PRD 的不变量，在独立文档中定义；**HTTP 接口**见 [docs/04-接口设计/](../04-接口设计/)。
